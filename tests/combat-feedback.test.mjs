...
function fixture(types){const model=new Combat();model.start();model.enemies=[];model.spawnClock=-10000;model.nextScrap=Infinity;model.slots=SLOT_Y.map(()=>null);model.carClocks.clear();for(let index=0;index<types.length;index++)if(types[index])addCar(model,types[index],index);return model;}
...
const earlyOffers=fixture(['cannon']);earlyOffers.supplyCount=1;earlyOffers.time=30;earlyOffers.supply();assert.ok(earlyOffers.availableMods().includes(earlyOffers.offers.find(offer=>offer.kind==='mod').id),'Early supply retains a valid optional modifier');
...
step(boss);assert.equal(boss.phase,'combat');assert.equal(boss.nextScrap,Infinity,'Milestone must wait for an earned supply');
boss.nextScrap=8;boss.scrap=8;boss.nextSupplyTime=boss.time+12;step(boss,359);assert.equal(boss.phase,'combat');
step(boss);assert.equal(boss.phase,'supply');assert.equal(boss.supplyCount,1);assert.equal(boss.nextScrap,28);
assert.ok(boss.offers.some(offer=>offer.kind==='mod'),'Milestone presentation is folded into the next normal supply');
assert.ok(boss.events.some(event=>event.type==='supply_offer'&&event.value==='1:milestone'));
